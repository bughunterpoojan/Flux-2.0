...
const Home = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute top-10 right-4 h-44 w-44 rounded-full bg-amber-200/60 blur-3xl" />

      {/* Header */}
      <nav className="flex items-center justify-between px-6 py-4 glass-panel sticky top-0 z-50 border-b border-primary-100/70">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to="/login?switch=1" className="btn-secondary px-5 py-2 text-slate-700">{t('login')}</Link>
          <Link to="/register" className="btn-primary px-5 py-2 text-white">{t('signup')}</Link>
        </div>
      </nav>

      {/* Hero Section */}
      <section className="relative py-24 px-6 bg-gradient-to-br from-primary-50/80 to-white overflow-hidden page-section">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-16">
          <div className="md:w-1/2 space-y-8 relative z-10">
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              <Link to="/register" className="btn-primary group flex items-center gap-2 px-8 py-4 text-lg">
      ...
      <section className="py-24 px-6 bg-white page-section">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="p-8 rounded-3xl bg-slate-50/80 hover:bg-white border border-transparent hover:border-primary-100 transition-all group hover:-translate-y-1 hover:shadow-xl hover:shadow-primary-100/60">
      ...
      {/* CTA Section */}
      <section className="py-24 px-6 bg-gradient-to-r from-primary-900 to-primary-700 page-section">
        <div className="max-w-5xl mx-auto text-center space-y-8">
          <h2 className="text-5xl font-extrabold text-white leading-tight">{t('cta_title')}</h2>
          <p className="text-xl text-primary-100">{t('cta_subtitle', { app: t('app_title') })}</p>
          <div className="flex justify-center gap-6 pt-6">
            <Link to="/register" className="btn-primary px-10 py-5 text-xl">{t('reg_farmer')}</Link>
            <Link to="/register" className="btn-secondary px-10 py-5 text-xl bg-white/95">{t('reg_buyer')}</Link>
          </div>
        </div>
      </section>

      {/* Footer */}
      <footer className="py-12 px-6 bg-white/90 border-t border-primary-100">
